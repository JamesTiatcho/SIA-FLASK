...
def extract_colors_from_image(image_path):
    ...
    print(f"Extracted average colors: {avg_colors}")
...
def predict_colors_cnn(img_path, skin_tone):
    try:
        ...
        print(f"Model outputs: {outputs.data}")
...
def get_color_labels(prediction, skin_tone):
    ...
    if skin_tone in color_labels:
        return color_labels[skin_tone][prediction % len(color_labels[skin_tone])]
    else:
        return ['Unknown']
...
def detect_skin_tone(image_path):
    try:
        ...
        print(f"Detected avg_rgb: {avg_rgb}")
        ...
        if avg_rgb[0] > 180:
            ...
            return 'Unknown', predict_colors_cnn(image_path, 'Unknown')
    except Exception as e:
        print(f"Error in detect_skin_tone: {e}")
        return 'Unknown', []
